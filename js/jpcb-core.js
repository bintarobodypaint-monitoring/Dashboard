...
const JPCB_MASTER = [
  ...
  '02.RJ PESAN PART','16.WAITING DELIVERY','18.JOB STOPPED','19.RAWAT JALAN',
...
function jpcbRawatCategory(unit){
  unit=unit||{};
  return String(
...
function isJpcbRawatJalanUnit(unit){
  return jpcbRawatCategory(unit)==='RJ';
}
...
function getJpcbRawatUnits(processes){
  ...
  function add(list,unit){
    unit=unit||{};
    if(jpcbRawatCategory(unit)!=='RJ') return;
    const key=String(
  ...
  // FILTER UTAMA: KATEGORI HARUS RJ.
  // Progress menentukan kolom, bukan menentukan apakah unit RJ.
  ...
  // 2) PROSES: WAITING PANEL REPAIR s/d PAINTING
  const prosesKeys=[
    '1.WAITING PANEL REPAIR','2.PANEL REPAIR',
    '3.WAITING PUTTY','4.PUTTY',
    '5.WAITING SURFACER','6.SURFACER',
    '7.WAITING PAINTING','8.PAINTING'
  ];
  prosesKeys.forEach(function(k){
    (Array.isArray(processes[k])?processes[k]:[]).forEach(function(u){add(proses,u);});
  });

  // 3) SIAP PASANG: HANYA WAITING REASSEMBLY
  const waitingReassembly=processes['11.WAITING REASSEMBLY'];
  if(Array.isArray(waitingReassembly)){
    waitingReassembly.forEach(function(u){add(siap,u);});
  }
  ...
  JPCB_SLIDES.forEach(function(slide,si){
    ...
    if(slide.rawatJalan){
      ...
      slide.processes.forEach(function(masterValue){
        const allUnits=Array.isArray(processes[masterValue])?processes[masterValue]:[];
        const units=allUnits.filter(function(unit){
          return jpcbRawatCategory(unit)!=='RJ';
        });
        total+=units.length;
