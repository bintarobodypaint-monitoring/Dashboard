...
function renderJPCB(data){
  ...
  JPCB_SLIDES.forEach(function(slide,si){
    ...
    slideEl.className='jpcb-slide'+(si===jpcbCurrentSlide?' active':'');
...
function findJPCBUnit(query){
  const q=String(query||'').toUpperCase().replace(/\s+/g,'').trim();
  ...
  if(!q){
    if(status)status.textContent='Siap mencari';
    return;
  }

  const cards=Array.from(document.querySelectorAll('#jpcbBoard .unit'));
  const card=cards.find(function(c){
    return [c.dataset.nopol||'',c.dataset.wo||'',c.dataset.pkb||'',c.textContent||'']
      .join(' ').toUpperCase().replace(/\s+/g,'').includes(q);
  });

  if(!card){
    if(status)status.textContent='Unit tidak ditemukan';
    return;
  }

  // Ambil data unit langsung dari kartu yang ditemukan.
  const unit={
    nopol:card.dataset.nopol||'',
    noPKB:card.dataset.pkb||'',
    group:card.dataset.group||'',
    type:card.dataset.type||'',
    sa:card.dataset.sa||'',
    progress:card.dataset.progress||'',
    tglMasuk:card.dataset.tglmasuk||'',
    janjiSelesai:card.dataset.janjiselesai||''
  };

  if(card.dataset.status==='bottom'){
    const list=card.closest('.status-list');
    if(list)list.scrollTo({
      left:Math.max(0,card.offsetLeft-(list.clientWidth-card.offsetWidth)/2),
      behavior:'smooth'
    });
  }else{
    showJpcbSlide(Number(card.dataset.slide||0),true);
    const list=card.closest('.unit-list');
    if(list)list.scrollTo({
      top:Math.max(0,card.offsetTop-(list.clientHeight-card.offsetHeight)/2),
      behavior:'smooth'
    });
  }

  if(status)status.textContent='Unit ditemukan ✓';

  // Bunyi "cling", tampilkan No Polisi besar di tengah,
  // lalu otomatis buka detail unit.
  showSearchFound(unit);

  setTimeout(function(){
    card.classList.remove('jpcb-found');
  },4200);
}

const jpcbInput=document.getElementById('jpcbSearchInput');
const jpcbSearchButton=document.getElementById('jpcbSearchButton');
function runJpcbSearch(e){
  if(e){
    e.preventDefault();
    e.stopPropagation();
  }
  const input=document.getElementById('jpcbSearchInput');
  findJPCBUnit(input ? input.value : '');
  return false;
}
if(jpcbInput){
  jpcbInput.addEventListener('keydown',function(e){
    if(e.key==='Enter')runJpcbSearch(e);
  });
}
if(jpcbSearchButton){
  jpcbSearchButton.addEventListener('click',runJpcbSearch);
}
