...
function jpcbRawatText(unit){
  unit=unit||{};
  const values=[
    unit.kategori,unit.KATEGORI,unit.category,unit.CATEGORY,
    unit.jenis,unit.JENIS,unit.tipe,unit.TIPE,
    unit.status,unit.STATUS,unit.rawatJalan,unit.rawat_jalan,
    unit['RAWAT JALAN'],unit['JENIS PERBAIKAN'],unit['JENIS SERVIS']
  ];
...
function isJpcbRawatJalanUnit(unit){
  const u=unit||{};
  const progress=String(u.progress||u.PROGRESS||'').trim().toUpperCase();
  if(progress.indexOf('RJ ')===0 || progress.indexOf('RAWAT JALAN')>=0) return true;
  const marker=jpcbRawatText(u);
  return marker.indexOf('RAWAT JALAN')>=0 || marker.indexOf('RAWATJALAN')>=0;
}
...
function getJpcbRawatUnits(processes){
  ...
  // 1) PESAN PART — khusus progress 02.RJ PESAN PART.
  const pesanKeys=['02.RJ PESAN PART','2.RJ PESAN PART'];
  pesanKeys.forEach(function(k){
    (Array.isArray(processes[k])?processes[k]:[]).forEach(function(u){add(pesan,u);});
  });

  // 2) PROSES — progress Rawat Jalan / RJ aktif selain Pesan Part.
  Object.keys(processes).forEach(function(k){
    const key=String(k).trim().toUpperCase();
    if(key.indexOf('RJ ')===0 || key.indexOf('RAWAT JALAN')>=0){
      if(key.indexOf('PESAN PART')>=0) return;
      (Array.isArray(processes[k])?processes[k]:[]).forEach(function(u){add(proses,u);});
    }
  });

  // 3) SIAP PASANG — unit Rawat Jalan yang sudah masuk WAITING REASSEMBLY.
  // Hanya unit yang terindikasi Rawat Jalan agar unit reguler tidak ikut masuk.
  const waitingReassembly=processes['11.WAITING REASSEMBLY'];
  if(Array.isArray(waitingReassembly)){
    waitingReassembly.forEach(function(u){
      if(isJpcbRawatJalanUnit(u)) add(siap,u);
    });
  }
