...
const QCReport=(function(){
 ...
 function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))} function pad(n){return String(n).padStart(2,'0')}
 ...
 function dk(r){return String(r&&r.tanggalISO||'').slice(0,10)} function today(){let d=new Date();return d.getFullYear()+'-'+pad(d.getMonth()+1)+'-'+pad(d.getDate())}
 ...
 function detail(key,date){let c=cfg[key],box=document.getElementById(c.detailId),rows=(data?.sections?.[key]?.rows||[]).filter(r=>!date||dk(r)===date);if(!date){box.innerHTML='<div class="qc-empty">Belum ada data QC pada bulan ini.</div>';return}let p=date.split('-'),title=p[2]+'/'+p[1]+'/'+p[0];if(!rows.length){box.innerHTML='<div class="qc-detail-title"><strong>'+esc(c.label)+' • '+title+'</strong><span>0 UNIT</span></div><div class="qc-empty">Tidak ada unit yang selesai QC pada tanggal tersebut.</div>';return}let h='<div class="qc-detail-title"><strong>'+esc(c.label)+' • '+title+'</strong><span>'+rows.length+' UNIT</span></div><div class="qc-table-wrap"><table class="qc-table"><thead><tr><th>NO</th><th>NO POLISI</th><th>NO PKB</th><th>TYPE</th><th>GROUP</th><th>SA</th><th>ASURANSI</th><th>PANEL</th><th>KERUSAKAN</th><th>HASIL QC</th><th>DEFECT</th><th>KETERANGAN</th><th>INSPECTOR</th></tr></thead><tbody>';rows.forEach((r,i)=>{h+='<tr><td>'+(i+1)+'</td><td>'+esc(r.noPolisi||'-')+'</td><td>'+esc(r.noPKB||'-')+'</td><td>'+esc(r.type||'-')+'</td><td>'+esc(r.group||'-')+'</td><td>'+esc(r.sa||'-')+'</td><td>'+esc(r.asuransi||'-')+'</td><td>'+esc(r.panel||'-')+'</td><td>'+esc(r.kerusakan||'-')+'</td><td>'+esc(r.hasilQC||'-')+'</td><td>'+esc(r.defect||'-')+'</td><td>'+esc(r.keterangan||'-')+'</td><td>'+esc(r.inspector||'-')+'</td></tr>'});h+='</tbody></table></div>';box.innerHTML=h}
 ...
 async function load(){
   ...
   setStatus('loading','Mengambil data QC...');
   const cb='qc_report_cb_'+Date.now();
   let finished=false,script=null,timer=null;
   ...
   window[cb]=function(j){finished=true;cleanup();if(!j||j.success===false){setStatus('error','API QC merespons error');return}data=j;render();setStatus('','Terhubung • update '+new Date().toLocaleTimeString('id-ID'))};
   script=document.createElement('script');
   script.async=true;
   script.src=base+(base.includes('?')?'&':'?')+'action=qcReport&month='+(cursor.getMonth()+1)+'&year='+cursor.getFullYear()+'&callback='+encodeURIComponent(cb)+'&_='+Date.now();
   script.onerror=function(){if(finished)return;cleanup();setStatus('error','Gagal memuat QC REPORT API');console.error('QC REPORT JSONP load error')};
   document.body.appendChild(script);
   timer=setTimeout(function(){if(finished)return;cleanup();setStatus('error','Timeout QC REPORT API')},15000);
 }
