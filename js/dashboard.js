(() => {
  ...
  function renderKpi(lead, pr, services, inap) {
    ...
    const currentInap = inap.length ? inap[inap.length - 1].total : 0;
    $("kInap").textContent = fmt(currentInap);

    const redo = num(services.redo?.totalRedo ?? services.redo?.total ?? 0);
    $("kRedo").textContent = services.redoOk ? fmt(redo) : "—";
  ...
  async function loadServices(month, year) {
    ...
    const [redoRes, qcRes] = await Promise.allSettled([
  ...
  async function load() {
    ...
    try {
      ...
      const [supplement, services, inap] = await Promise.all([
        loadJscbSupplement(),
      ...
      renderKpi(lead, pr, services, inap);
