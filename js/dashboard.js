(() => {
  ...
  async function loadUnitInapDaily(month, year) {
    ...
    const effectiveLast = isCurrentMonth ? Math.min(last, now.getDate()) : last;
    const dates = Array.from({ length: effectiveLast }, (_, i) => new Date(year, month, i + 1));
    ...
    inapDaily = Array.from({ length: last }, (_, i) => ({
      date: new Date(year, month, i + 1),
      day: i + 1,
      total: i < effectiveLast ? result[i] : 0
    }));
    return inapDaily;
  }
  ...
  async function loadJscbSupplement() {
    const result = { completionMap: {}, rekap: null, ok: false };
    try {
      // Dashboard hanya membutuhkan REKAP untuk tanggal selesai/delivery.
      // jpcbLive tidak diperlukan di sini dan sengaja tidak dipanggil agar loading lebih cepat.
      const rekap = await getApiJson(JSCB_API + "?action=rekap&t=" + Date.now());
      result.rekap = rekap;
  ...
  async function load() {
    ...
    const status = $("status");
    status.textContent = "Mengambil data utama dashboard...";

    // Bersihkan chart lama supaya perpindahan bulan tidak meninggalkan tampilan lama.
    Object.keys(charts).forEach(destroy);

    try {
      // PENTING: grafik Unit Inap harian TIDAK lagi menahan proses render dashboard.
      // Sebelumnya dashboard menunggu sampai 31 request API selesai sehingga terlihat lama.
      const [unitRes, supplement, services, latestInap] = await Promise.all([
        getApiJson(API + "?action=unit&t=" + Date.now()),
        loadJscbSupplement(),
      ...
      status.textContent = `Data ${String(month).padStart(2, "0")}/${year} • ${fmt(units.length)} unit • Leadtime: ${leadSource} • memuat grafik Unit Inap...`;
      window.__dashboardLead = lead;

      // Grafik Unit Inap dimuat SETELAH dashboard tampil.
      // Hanya tanggal sampai hari ini untuk bulan berjalan; tanggal masa depan tetap 0.
      loadUnitInapDaily(month - 1, year)
        .then(() => {
          renderInapDaily();
          status.textContent = `Data ${String(month).padStart(2, "0")}/${year} • ${fmt(units.length)} unit • Dashboard siap • Unit Inap harian selesai dimuat.`;
        })
        .catch(e => {
          console.warn("Grafik Unit Inap", e);
          status.textContent = `Data ${String(month).padStart(2, "0")}/${year} • Dashboard siap • Grafik Unit Inap gagal dimuat.`;
        });
    } catch (e) {
